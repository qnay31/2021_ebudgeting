...
$q2    = mysqli_query($conn, "SELECT * FROM galeri_humas WHERE nomor_id = '$unik' AND program = '$program' ");
$i = 1;
$sql2 = $q2;
...
while ($data2 = mysqli_fetch_array($sql2)) {
    ...
    unlink("../../img/laporan/humas/".$total_dokumentasi[$i]);
}
...
// die(var_dump($hapus));
$query2 = mysqli_query($conn, "UPDATE `humas` SET 
`tgl_pemakaian`='',
`deskripsi_pemakaian`='',
`dana_terpakai`='',
...
$query3 = mysqli_query($conn, "DELETE FROM `galeri_humas` WHERE nomor_id = '$unik' AND program = '$program'");
...
document.location.href = '../../user/$_SESSION[username].php?id_input=laporan_humas';
</script>";
} else {
echo "<script>

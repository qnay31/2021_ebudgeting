...
$q2    = mysqli_query($conn, "SELECT * FROM galeri_produksi WHERE nomor_id = '$unik' AND program = '$program' ");
$i = 1;
$sql2 = $q2;
...
while ($data2 = mysqli_fetch_array($sql2)) {
    ...
    unlink("../../img/laporan/produksi/".$total_dokumentasi[$i]);
}
...
// die(var_dump($hapus));
$query2 = mysqli_query($conn, "UPDATE `produksi` SET 
`tgl_pemakaian`='',
`deskripsi_pemakaian`='',
`dana_terpakai`='',
`laporan`='Belum Laporan'

WHERE id = '$unik' AND produksi = '$program' ");

$query3 = mysqli_query($conn, "DELETE FROM `galeri_produksi` WHERE nomor_id = '$unik' AND program = '$program'");
...
$result2 = mysqli_query($conn, "INSERT INTO log_aktivity VALUES('', '$_SESSION[nama]', '$posisi', '$ip', '$date', '$_SESSION[nama] Divisi $posisi Telah Menghapus Laporan $program')");
    ...
    document.location.href = '../../user/$_SESSION[username].php?id_input=laporan_produksi';
    </script>";
    } else {
    echo "<script>

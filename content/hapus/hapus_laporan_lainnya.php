...
$q2    = mysqli_query($conn, "SELECT * FROM galeri_lainnya WHERE nomor_id = '$unik' AND program = '$program' ");
$i = 1;
$sql2 = $q2;
...
while ($data2 = mysqli_fetch_array($sql2)) {
    ...
    unlink("../../img/laporan/lainnya/".$total_dokumentasi[$i]);
}

// die(var_dump($hapus));
$query2 = mysqli_query($conn, "UPDATE `anggaran_lain` SET 
`tgl_laporan`='',
`pemakaian`='',
`dana_terpakai`='',
...
$query3 = mysqli_query($conn, "DELETE FROM `galeri_lainnya` WHERE nomor_id = '$unik' AND program = '$program'");

if (($query2 == true )) {

    echo "<script>
alert('Data Berhasil Dihapus');
document.location.href = '../../user/$_SESSION[username].php?id_input=laporan_logistik&id_management=lainnya';
</script>";
} else {
echo "<script>

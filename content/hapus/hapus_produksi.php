...
// die(var_dump($hapus));
$query2 = mysqli_query($conn, "DELETE FROM `produksi` WHERE id = '$unik' AND produksi = '$kategori' AND MONTH(tgl_pengajuan) = '$periode' ");
...
if ($id_pengurus  == "kepala_produksi") {
    ...
    document.location.href = '../../user/$_SESSION[username].php?id_input=verifikasi_produksi';
    </script>";
    }  else {
    echo "<script>
    ...
    if (($query2 == true)) {
        echo "<script>
    alert('Data Berhasil Dihapus');
    document.location.href = '../../user/$_SESSION[username].php?id_link_logistik=produksi&id_input=verifikasi_produksi';
    </script>";
    }  else {
    echo "<script>
    alert('Data Tidak Berhasil Dihapus');

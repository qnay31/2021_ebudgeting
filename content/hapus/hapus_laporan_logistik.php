...
$q2    = mysqli_query($conn, "SELECT * FROM galeri_logistik WHERE nomor_id = '$unik' AND program = '$program' ");
$i = 1;
$sql2 = $q2;
...
while ($data2 = mysqli_fetch_array($sql2)) {
    ...
    unlink("../../img/laporan/logistik/".$total_dokumentasi[$i]);
}
...
// die(var_dump($hapus));
$query2 = mysqli_query($conn, "UPDATE `logistik` SET 
`tgl_pemakaian`='',
`deskripsi_pemakaian`='',
`dana_terpakai`='',
...
$query3 = mysqli_query($conn, "DELETE FROM `galeri_logistik` WHERE nomor_id = '$unik' AND program = '$program'");
...
if ($query2 !== true && $_SESSION[id_pengurus] == "kepala_logistik") {
    echo "<script>
    alert('Data Tidak Berhasil Dihapus');
    document.location.href = '../../user/$_SESSION[username].php?id_check=logistik_laporan';
    </script>";

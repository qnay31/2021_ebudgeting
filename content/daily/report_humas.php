...
if (isset($_POST["input"]) ) {
    ...
    if(daily_report($_POST) > 0 ) {
        echo "<script>
                ...
                document.location.href = '$link.php?id_input=daily_humas';
            </script>";
...
if ($pengurus_id == "ketua_yayasan") {
    ...
    FROM daily_report
    JOIN list_divisi ON list_divisi.id_pengurus = daily_report.id_pengurus WHERE list_divisi.akses = 'humas' ORDER BY `id` DESC ");

    $s = $q->num_rows;


    // program depok
} elseif ($pengurus_id == "kepala_humas" && $_SESSION['cabang'] == "Depok") {
    $q = mysqli_query($conn, "SELECT daily_report.nama, daily_report.id, daily_report.id_pengurus, daily_report.posisi, daily_report.cabang, daily_report.aktivitas, daily_report.tgl_aktivitas, daily_report.deskripsi, list_divisi.posisi 
    FROM daily_report
    JOIN list_divisi ON list_divisi.id_pengurus = daily_report.id_pengurus WHERE list_divisi.akses = 'humas' AND daily_report.cabang = '$_SESSION[cabang]' ORDER BY `id` DESC ");
...
<div class="container-fluid">
    ...
    <div class="col-xl-12 col-lg-12">
        ...
        <div class="card shadow">
            <div class="card-body">
                <?php if ($pengurus_id == "ketua_yayasan") { ?>
                <label for=""><b>Daily Aktivifas Humas</b>
                    <hr>
                </label>
                <?php } else { ?>

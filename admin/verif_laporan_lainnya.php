...
$unik     = $_GET["id_unik"];
$periode  = $_GET["id_p"]; 
$query  = mysqli_query($conn, "SELECT * FROM anggaran_lain WHERE id = '$unik' AND MONTH(tgl_laporan) = '$periode' ");
...
// Global
$q  = mysqli_query($conn, "SELECT * FROM anggaran_lain WHERE MONTH(tgl_laporan) = '$periode' AND laporan = 'Terverifikasi' ");
...
// cek nama
$c_query = mysqli_query($conn, "SELECT bulan FROM data_lainnya WHERE bulan = '$bulan' ");

    if (mysqli_fetch_assoc($c_query)) {
        $tes = mysqli_query($conn, "UPDATE `data_lainnya` SET 
...
document.location.href = '../user/$_SESSION[username].php?id_laporan=check_lainnya';
</script>";
} else {
echo "<script>

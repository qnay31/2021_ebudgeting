...
$unik     = $_GET["id_unik"];
$periode  = $_GET["id_p"]; 
$query  = mysqli_query($conn, "SELECT * FROM logistik WHERE id = '$unik' AND MONTH(tgl_pemakaian) = '$periode' ");
...
$kueri = mysqli_query($conn, "SELECT akun_pengurus.id_pengurus, akun_pengurus.nama, list_divisi.posisi, list_divisi.akses 
FROM akun_pengurus
...
// Global
$q  = mysqli_query($conn, "SELECT * FROM logistik WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' ");
...
// logistik depok
$q2  = mysqli_query($conn, "SELECT * FROM logistik WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' AND cabang = 'Depok' ");
...
// logistik bogor
$q3  = mysqli_query($conn, "SELECT * FROM logistik WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' AND cabang = 'Bogor' ");
...
// cek nama
$c_query = mysqli_query($conn, "SELECT bulan FROM data_logistik WHERE bulan = '$bulan' ");
...
if ($id_pengurus == 'logistik' && $cabang == 'Depok') {
    if (mysqli_fetch_assoc($c_query)) {
        $tes = mysqli_query($conn, "UPDATE `data_logistik` SET 
        ...
        if (mysqli_fetch_assoc($c_query)) {
            $tes2 = mysqli_query($conn, "UPDATE `data_logistik` SET 

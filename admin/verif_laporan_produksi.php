...
$unik     = $_GET["id_unik"];
$periode  = $_GET["id_p"]; 
$query  = mysqli_query($conn, "SELECT * FROM produksi WHERE id = '$unik' AND MONTH(tgl_pemakaian) = '$periode' ");
...
$result2 = mysqli_query($conn, "INSERT INTO log_aktivity VALUES('', '$_SESSION[nama]', '$posisi', '$ip', '$date', '$_SESSION[nama] Divisi $posisi Telah Mengkonfirmasi Laporan $program $cabang')");
...
// Global
$q  = mysqli_query($conn, "SELECT * FROM produksi WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' ");
...
// produksi depok
$q2  = mysqli_query($conn, "SELECT * FROM produksi WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' AND cabang = 'Depok' ");
...
// produksi bogor
$q3  = mysqli_query($conn, "SELECT * FROM produksi WHERE MONTH(tgl_pemakaian) = '$periode' AND laporan = 'Terverifikasi' AND cabang = 'Bogor' ");
...
// cek nama
$c_query = mysqli_query($conn, "SELECT bulan FROM data_produksi WHERE bulan = '$bulan' ");


// sub cabang logistik
// logistik depok
if ($id_pengurus == 'kepala_produksi' && $cabang == 'Depok') {
    if (mysqli_fetch_assoc($c_query)) {
        $tes = mysqli_query($conn, "UPDATE `data_produksi` SET 
            `anggaran_produksi_depok`       ='$hasil_anggaran2',
            `terpakai_produksi_depok`       ='$hasil_terpakai2',
        ...
        // logistik_bogor
    } elseif ($id_pengurus == 'kepala_logistik' && $cabang == 'Bogor') {
        if (mysqli_fetch_assoc($c_query)) {
            $tes2 = mysqli_query($conn, "UPDATE `data_produksi` SET 
            `anggaran_produksi_bogor`       ='$hasil_anggaran3',
            `terpakai_produksi_bogor`       ='$hasil_terpakai3',
            `anggaran_global`               ='$hasil_anggaran',
            `terpakai_global`               ='$hasil_terpakai'
            WHERE bulan = '$bulan' ");   
        }
...
document.location.href = '../user/$_SESSION[username].php?id_laporan=check_produksi';
</script>";
} else {
echo "<script>
